...
{
    public class SetUserBudget
    {
        public static int GetBudgetHours(int user)
        {
            var totalMinutes = UserBudget.GetBudgetAmount(user);
        ...
        public static void SetBudgetAmount(int hours, int minutes, int user)
        {
            using (var db = new Database())
            {
                ...
                {
                    records.ElementAt(0).Monthly = totalMinutes;
                }
        ...
        public static void Save(int bmHours, int bmMinutes, int bdHours, int bdMinutes, int user)
        {
            SetBudgetAmount(bmHours, bmMinutes, user);
            SetUserBudgetToday.SetBudgetAmount(bdHours, bdMinutes, user);
        }
    }
}
