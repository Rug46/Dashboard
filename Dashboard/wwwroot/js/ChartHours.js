    ...
    var hrt_chart = new Chart(hrt_ctx, {
        ...
        options: {
            ...
            onClick: function (event) {
                ...
                $.ajax({
                    url: "/Chart/GetRecordsHours",
                    dataType: "json",
                    data: { hoursAgo: 0, index: 0, col: 1 },
                    success: function (blah){

                        console.log(blah)
                        console.log(blah.result)
                        $("#test-id").text(blah.result);
                    }
                });
...
// hours_today onClick
document.getElementById("hours_today").onclick = function (evt) {
    var activePoints = hrt_chart.getElementAtEvent(evt);
